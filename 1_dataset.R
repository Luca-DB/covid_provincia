...
#    this is just a backup to check the code without internet connection.
...
# -- you can either read data from path_prov (requires an internet connection)
#    or read: "dati/covid_data_bck.csv" (the previous back-up plan).
c19_pr <- read_csv(path_prov,
                   col_types = cols(col_character(), col_character(),
                                    ...
                                    .default = col_skip())) %>%
# -- below there is the back-up plan. You should silence previous lines.
## c19_pr <- read_csv2("dati/covid_data_bck.csv") %>%
  mutate(date=as.Date(data),
         ...
         totale_casi = ifelse(nuts3=="Bolzano Bozen" &
                                date>=ymd("2021-03-22"),
# -- Bolzano informs that 10665 molecular tests performed before 15/01/2021 were not counted until 03/21/2021.
...
# we will now adjust negative daily incidences with a 7-days moving average.
...
#    It refers to the first wave (March 2020) we are not analyzing.
...
# -- cleaning IRPEF data. The parsing issues come from a column of ";"
#    at the end of the dataset and it is nothing to worry about.
irpef <- read_csv2(
...
# -- grouping municipality level data at the province level
#    and then the two lowest income classes into one
irpef_prov <- irpef %>% 
...
# * tiers' colors ---------------------------------------------------------
# -- no known dataset for this. All informations were gathered by the authors
#    from: https://tg24.sky.it/archivio
